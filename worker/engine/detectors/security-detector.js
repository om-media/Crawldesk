...
exports.detectSecurityIssues = detectSecurityIssues;
/**
 * Detects security header issues and mixed content on HTTPS pages.
 */
function detectSecurityIssues(result) {
    const issues = [];
    if (!result.seo)
        return issues;
    ...
    if (isHttps && result.links) {
        const httpResources = result.links.filter((l) => (l.linkType === 'image' || l.linkType === 'script' || l.linkType === 'css')
