...
exports.detectHreflangIssues = detectHreflangIssues;
/**
...
function extractHreflangs(html) {
    ...
    while ((match = revRegex.exec(html)) !== null) {
        ...
        if (entries.some(e => e.href === href && e.hreflang === hreflang))
            continue;
        const mediaMatch = match[0].match(/media=["']([^"']+)["']/i);
        entries.push({
            hreflang,
...
function detectHreflangIssues(result) {
    const issues = [];
    if (!result.seo?.hreflangs || result.seo.hreflangs.length === 0)
        return issues;
...
//# sourceMappingURL=hreflang-detector.js.map