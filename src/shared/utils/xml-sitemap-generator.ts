...
import type { GenerateSitemapInput, GeneratedSitemapResult } from '../types/sitemap-generator'
...
function parseImagesField(rawJson?: string | null): Array<{ loc: string }> {
  if (!rawJson) return []
  try {
    const parsed = JSON.parse(rawJson) as Array<Record<string, unknown>>
    return parsed
      .map((img: Record<string, unknown>) => ({ loc: String(img.src ?? img.loc ?? '') }))
      .filter((img) => img.loc)
  } catch {
    return []
  }
...
function urlBlock(urlRow: NonNullable<GenerateSitemapInput['urls']>[number], includeImages: boolean): string {
...
export function generateSitemap(input: GenerateSitemapInput): GeneratedSitemapResult {
  ...
  const filtered = input.urls.filter((row) => {
    if (onlyHttp200 && (row.status_code == null || row.status_code < 200 || row.status_code >= 300)) return false
    if (onlyIndexable && row.indexability !== 'indexable') return false
    return true
  })
