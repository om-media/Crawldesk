...
function setupCrawldesk() {
  ...
  window.crawldesk = {
    ...
    exports: {
      exportUrls: async (input: any) => {
        ...
        const result = await invoke('export_urls_csv', {
          ...
          filterStatusCategory: input.filters?.statusCategory,
          filterStatusCode: input.filters?.statusCode,
          excludedStatusCodes: input.filters?.excludedStatusCodes,
          search: input.filters?.search,
          sortBy: input.sort?.field,
          sortOrder: input.sort?.direction,
        })
      ...
      exportKeywords: async (input: any) => {
        const crawlId = input.crawlId ?? input.projectId
        const result = await invoke('export_keywords_csv', {
          crawlId: toId(crawlId),
          outputPath: await exportPath('keywords', crawlId),
          gramType: input.gramType,
          search: input.filters?.search,
        })
        return normalizeExportResult(result)
      },
      exportContentAudit: async (input: any) => {
        const crawlId = input.crawlId ?? input.projectId
        const result = await invoke('export_content_audit_csv', {
          crawlId: toId(crawlId),
          outputPath: await exportPath('content-audit', crawlId),
          search: input.filters?.search,
        })
        return normalizeExportResult(result)
      },
    ...
    schedules: {
      ...
      runNow: (id: string | number) => invoke('run_crawl_schedule_now', { id: toId(id) }),
    },
    diff: {
