    ...
    <aside className="w-[240px] min-w-[240px] bg-sidebar border-r border-lumen flex flex-col h-full">
      ...
      <div className="px-5 pt-6 pb-2">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-teal-accent flex items-center justify-center relative overflow-hidden">
      ...
      <div className="px-3 py-2">
        <button
          ...
          className={`w-full text-left px-3 py-2 rounded-xl text-sm transition-all ${currentRoute === 'projects' ? 'bg-teal-bg border border-lumen' : 'hover:bg-midnight/40'} border-transparent`}
        >
        ...
        {projectNavItems.map(item => {
          ...
          return (
            <button
              ...
              className={`relative w-full flex items-center gap-3 pl-3 pr-3 py-2.5 rounded-[12px] text-sm transition-all ${isActive ? 'bg-teal-bg' : 'hover:bg-midnight/30'} ${!selectedProjectId ? 'opacity-40 cursor-not-allowed' : ''}`}
            >
              {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-teal-accent" />}
              <span className={`${isActive ? 'text-teal-accent' : 'text-primary-muted'} text-lg leading-none`}>{item.icon}</span>
      ...
      <div className="p-3 border-t border-lumen/60">
        <button
          ...
          className={`relative w-full flex items-center gap-3 pl-3 pr-3 py-2.5 rounded-[12px] text-sm transition-all ${currentRoute === 'settings' ? 'bg-teal-bg' : 'hover:bg-midnight/30'}`}
        >
          {currentRoute === 'settings' && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-teal-accent" />}
          <span className={currentRoute === 'settings' ? 'text-teal-accent' : 'text-primary-muted'} style={{ fontSize: '16px' }}>⚙</span>
