...
import type { RenderedPageData } from '../js-renderer'
 ...
 */
export function detectJsRenderingIssues(
  result: PageResult,
  rendered?: RenderedPageData | null
): IssueRecordInput[] {
  const issues: IssueRecordInput[] = []
  if (!rendered || !result.seo) return issues
