...
export interface CarbonResult {
  ...
  energyWh: number
  /** CO2 emitted in grams */
  co2Grams: number
  /** Rating: green / yellow / red based on thresholds */
...
export function estimateCarbon(contentLength: number, includeTransfer = true): CarbonResult {
  ...
  if (co2Grams < GREEN_THRESHOLD) rating = 'green'
  else if (co2Grams < YELLOW_THRESHOLD) rating = 'yellow'
  else rating = 'red'
