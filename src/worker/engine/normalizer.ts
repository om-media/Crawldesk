...
export function normalizeUrl(input: string, baseUrl?: string): NormalizedUrlResult {
  try {
    ...
    else if (parsed.protocol === 'https:' && parsed.port === '443') parsed.port = ''
...
function cleanQueryParams(search: string): string {
  if (!search) return ''
