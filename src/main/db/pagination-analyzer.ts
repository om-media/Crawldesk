...
export class PaginationAnalyzer {
  ...
  // ----------------------------------------------------------------
  private detectBrokenPaginationChains(crawlId: string): void {
    ...
    `).all(crawlId) as PaginationRow[]

    if (rows.length === 0) return

    // Build lookup sets for crawled normalized URLs
    const crawledUrls = new Map<string, PaginationRow>(rows.map(r => [r.normalized_url.toLowerCase(), r]))
    ...
    const flaggedForChainBreak = new Set<string>()
    const flaggedForLoop = new Set<string>()
        ...
        try {
          ...
          if (!crawledUrls.has(nextNormed)) {
            ...
            if (!flaggedForChainBreak.has(row.id)) {
              insertIssue.run(
                crypto.randomUUID(), crawlId, row.id, row.url,
                'pagination_broken_loop',
                'medium',
                `rel="next" points to ${row.pagination_next} which was not crawled.`,
                'Ensure pagination links point to valid pages within the same paginated series.',
                now
              )
          ...
          if (!crawledUrls.has(prevNormed)) {
            if (!flaggedForChainBreak.has(row.id)) {
              insertIssue.run(
                ...
                'pagination_broken_loop',
                'medium',
                `rel="prev" points to ${row.pagination_prev} which was not crawled.`,
                'Ensure pagination links point to valid pages within the same paginated series.',
                now
              )
              flaggedForChainBreak.add(row.id)
    ...
    const visitedInLoopCheck = new Set<string>()
    for (const starter of rows) {
      if (visitedInLoopCheck.has(starter.normalized_url)) continue
    ...
    for (const starter of rows) {
      if (visitedRev.has(starter.normalized_url)) continue
      const chainIds: string[] = []
      const chainSet = new Set<string>()
  ...
  private followNextChain(
    starter: PaginationRow, crawledUrls: Map<string, PaginationRow>, visited: Set<string>,
    chainIds: string[], chainSet: Set<string>, insertIssue: any, crawlId: string,
    rows: PaginationRow[], flaggedForLoop: Set<string>, now: string
  ): void {
    let node: PaginationRow | null = starter
    while (node?.pagination_next && !visited.has(node.normalized_url)) {
      if (chainSet.has(node.normalized_url)) {
        for (const nid of chainIds) {
          if (!flaggedForLoop.has(nid)) {
            insertIssue.run(
              crypto.randomUUID(), crawlId, nid,
              rows.find((r: PaginationRow) => r.id === nid)?.url ?? '',
              'pagination_broken_loop',
              'medium',
              `This page is part of a circular pagination chain.`,
              'Pagination should be linear (page 1 → page 2 → ...), not circular.',
              now
            )
      ...
      const pn = node.pagination_next as string
      chainSet.add(nu)
    ...
    for (const n of chainSet) visited.add(n)
  }

  private followPrevChain(
    starter: PaginationRow, crawledUrls: Map<string, PaginationRow>, visited: Set<string>,
    chainIds: string[], chainSet: Set<string>, insertIssue: any, crawlId: string,
    rows: PaginationRow[], flaggedForLoop: Set<string>, now: string
  ): void {
    ...
    while (node?.pagination_prev && !visited.has(node.normalized_url)) {
      ...
      const nu = node.normalized_url
      const pp = node.pagination_prev as string
      chainSet.add(nu)
      chainIds.push(node.id)
      try {
  ...
  // ----------------------------------------------------------------
  private detectMissingPaginationCanonical(crawlId: string): void {
    const now = new Date().toISOString()
    ...
    `).all(crawlId) as Array<{ id: string; url: string; normalized_url: string; canonical: string | null }>
    ...
    for (const row of rows) {
      insertIssue.run(
        crypto.randomUUID(), crawlId, row.id, row.url,
        'missing_pagination_canonical',
        'medium',
        `Paginated page ${row.canonical ? `has canonical "${row.canonical}"` : 'has no canonical tag'}.`,
        'Add a self-referencing <link rel="canonical"> to each paginated URL to prevent duplicate content issues.',
        now
      )
    }
  }
