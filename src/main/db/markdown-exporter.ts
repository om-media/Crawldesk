...
/** Options for markdown export (Feature 6.3). */
interface ExportOptions {
  /** 'single_file' for one report or 'per_page' for individual markdown files per URL */
  format?: 'single_file' | 'per_page'
  /** Include YAML front matter with crawl metadata in each file */
  yamlFrontMatter?: boolean
  /** CSS selectors to exclude from page content when exporting per-page mode */
...
export class MarkdownExporter {
  ...
  exportCrawl(crawlId: string, outputPathOrDir?: string, options?: any): string | string[] {
  ...
  private exportSingleFile(crawlId: string, outputPath?: string, yamlFrontMatter = false): string {
    ...
    `).all(crawlId) as Array<{
      url: string; status_code?: number | null; title?: string | null; meta_description?: string | null
      word_count?: number | null; h1?: string | null; canonical?: string | null; robots_meta?: string | null
    }>
    ...
    `).all(crawlId) as Array<{ url: string; issue_type: string; severity: string; message: string }>
    ...
    const issueByType: Map<string, Array<{ url: string; message: string }>> = new Map()
    for (const iss of issues) {
      if (!issueByType.has(iss.issue_type)) issueByType.set(iss.issue_type, [])
  ...
  private exportPerPage(crawlId: string, outputDir?: string, yamlFrontMatter = false): string[] {
    const urls = this.db.prepare(`
      SELECT id, url, status_code, title, meta_description, word_count, h1, canonical, robots_meta
      FROM urls WHERE crawl_id = ? ORDER BY CASE WHEN status_code >= 500 THEN 0 WHEN status_code >= 400 THEN 1 ELSE 2 END, status_code, url
    `).all(crawlId) as Array<{
      id: string; url: string; status_code?: number | null; title?: string | null; meta_description?: string | null
      word_count?: number | null; h1?: string | null; canonical?: string | null; robots_meta?: string | null
    }>

    const issuesByURL = new Map<string, Array<{ issue_type: string; severity: string; message: string }>>()
    const allIssues = this.db.prepare(`
      SELECT url, issue_type, severity, message FROM issues WHERE crawl_id = ?
    `).all(crawlId) as Array<{ url: string; issue_type: string; severity: string; message: string }>
    for (const iss of allIssues) {
      if (!issuesByURL.has(iss.url)) issuesByURL.set(iss.url, [])
    ...
    for (const u of urls) {
      ...
      if (u.canonical) md += `**Canonical:** ${u.canonical}\n`
      md += `\n`
