 ...
 */
export class PostCrawlAnalyzer {
   ...
   */
  async analyze(crawlId: string): Promise<void> {
  ...
  // ----------------------------------------------------------------
  private detectDuplicateContent(crawlId: string): void {
    ...
    `).all(crawlId) as Array<{ content_hash: string; url_ids: string; cnt: number }>
    ...
    if (exactGroups.length > 0) {
      ...
      for (const group of exactGroups) {
        ...
        const firstUrlRow = this.db.prepare(
          'SELECT url FROM urls WHERE id = ?'
        ).get(ids[0]) as { url: string } | undefined

        for (const urlId of ids) {
          const urlRow = this.db.prepare(
            'SELECT url FROM urls WHERE id = ?'
          ...
          insertIssue.run(
            ...
            'high',
            `${group.cnt} page(s) share identical content.`,
            'Consolidate duplicate pages using canonical tags or merge content onto a single authoritative URL.',
            now
          )
        }
      }
    ...
    `).all(crawlId) as Array<{ prefix: string; url_ids: string; cnt: number }>
    ...
    for (const row of nearDupRows) {
      ...
      const hashes = ids.map(id => {
        const r = this.db.prepare('SELECT content_hash FROM urls WHERE id = ?').get(id) as { content_hash?: string } | undefined
        return r?.content_hash
      })
      const uniqueFullHashes = new Set(hashes.filter(Boolean)).size
      if (uniqueFullHashes <= 1) continue // All same full hash → already reported as exact duplicate
      ...
      for (const urlId of ids) {
        ...
        insertIssue.run(
          ...
          'medium',
          `Page has near-duplicate content with ${row.cnt - 1} other page(s).`,
          'Review these pages for substantial differences. If too similar, consider merging or differentiating content.',
          now
        )
      }
    }
  ...
  // ----------------------------------------------------------------
  private detectCanonicalChains(crawlId: string): void {
    const now = new Date().toISOString()
    ...
    `).all(crawlId) as Array<{ normalized_url: string; canonical: string; url: string }>

    if (rows.length < 2) return

    const canonMap = new Map<string, string>() // normUrl → canonical
    const normToOriginalUrl = new Map<string, string>() // normUrl → original crawled URL
    ...
    for (const startNorm of canonMap.keys()) {
      if (!visited.has(startNorm)) {
        const chain: string[] = [startNorm]
        let current = startNorm
        while (true) {
          const next = canonMap.get(current)
          if (!next || next === current) break // Self-referencing or no canonical — end of chain
          chain.push(next)
          current = next
          if (chain.length > 10) break // Safety limit
        ...
        if (chain.length >= 3) {
          ...
          const startUrlId = this.db.prepare(
            'SELECT id FROM urls WHERE crawl_id = ? AND normalized_url = ? LIMIT 1'
          ).get(crawlId, startNorm) as { id?: string } | undefined
          if (startUrlId?.id) {
            insertIssue.run(
              crypto.randomUUID(),
              crawlId,
              startUrlId.id,
              normToOriginalUrl.get(startNorm) ?? '',
              'canonical_chain',
              'medium',
              `Canonical chain detected: ${chainUrls} (${chain.length - 1} hops).`,
              'Update direct links to point to the final canonical URL and use self-referencing canonicals on intermediate pages.',
              now
            )
          }
        } else {
