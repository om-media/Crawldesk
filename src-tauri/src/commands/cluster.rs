...
pub fn find_clusters(crawl_id: i64) -> Result<Vec<ContentCluster>, String> {
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, url, title, meta_description, h1, headings_h2, headings_h3, extractable_text, seo_data_json
             FROM urls
             WHERE crawl_id = ?1
               AND (
    ...
    for row_result in rows.filter_map(|r| r.ok()) {
        let (url_id, url, title, meta, h1, headings_h2, headings_h3, extractable, seo_json) =
            row_result;
...
mod tests {
    ...
    fn extract_text_uses_normalized_columns_without_json() {
        let text = extract_text_from_fields(
            ...
            &Some("Column body text for clustering".to_string()),
            &None,
        );
