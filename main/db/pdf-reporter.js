...
class PdfReporter {
    ...
    async generate(crawlId) {
        // Gather data
        const crawl = this.db.prepare('SELECT * FROM crawls WHERE id = ?').get(crawlId);
        if (!crawl)
    ...
    `).get(crawlId);
        const issuesBySeverity = this.db.prepare("SELECT severity, COUNT(*) as cnt FROM issues WHERE crawl_id = ? GROUP BY severity").all(crawlId);
        const topIssues = this.db.prepare('SELECT issue_type, COUNT(*) as cnt FROM issues WHERE crawl_id = ? GROUP BY issue_type ORDER BY cnt DESC LIMIT 10').all(crawlId);
        ...
        try {
            ...
            const outputDir = path_1.default.join(this.getDataDir(), 'reports');
            if (!fs_1.default.existsSync(outputDir))
                fs_1.default.mkdirSync(outputDir, { recursive: true });
            const filePath = path_1.default.join(outputDir, `report-${crawlId}.pdf`);
    ...
    buildReportHtml(data) {
        const issuesBars = data.issuesBySeverity.map((i) => {
        ...
        const topIssuesRows = data.topIssues.map((t) => `<tr><td style="padding:6px;border-bottom:1px solid #eee;text-align:left;font-family:monospace">${this.escapeHtml(t.issue_type)}</td><td style="padding:6px;border-bottom:1px solid #eee;text-align:center">${t.cnt}</td></tr>`).join('');
        return `<!DOCTYPE html>
...
exports.PdfReporter = PdfReporter;
