...
async function runSmoke() {
  ...
  try {
    ...
    record('release crawl schedules CRUD works', result.scheduleEdited?.start_url?.endsWith('/about') && result.scheduleEdited?.cron_expression === '0 */6 * * *' && result.scheduleUpdated?.enabled === 0 && result.scheduleUpdated?.next_run_at == null && result.schedulesBeforeDelete.length === 1 && result.schedulesAfterDelete.length === 0, JSON.stringify({ edited: result.scheduleEdited, updated: result.scheduleUpdated, before: result.schedulesBeforeDelete, after: result.schedulesAfterDelete }))
